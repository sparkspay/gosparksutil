...
import (
	...
	"github.com/sparkspay/gosparks/chaincfg/chainhash"
	"github.com/sparkspay/gosparks/wire"
...
type CoinSet struct {
	coinList      *list.List
	totalValue    gosparksutil.Amount
	totalValueAge int64
}
...
func (cs *CoinSet) TotalValue() (value gosparksutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue gosparksutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue gosparksutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount gosparksutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue gosparksutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue gosparksutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue gosparksutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue gosparksutil.Amount, coins []Coin) (Coins, error) {
...
// gosparksutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() gosparksutil.Amount {
	return gosparksutil.Amount(c.txOut().Value)
}
