...
import (
	...
	"github.com/sparkspay/gosparksutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := gosparksutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(gosparksutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := gosparksutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := gosparksutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := gosparksutil.NewBlockFromBytes(blockBytes)
	if err != nil {
